...
TM_BMP180_Result_t TM_BMP180_Init(I2C_HandleTypeDef *i2c ,TM_BMP180_t* BMP180_Data) {
	uint8_t data[22];
	uint8_t i = 0;
	bmp_hi2c1 = i2c;
	
	/* Test if device is connected */
	if (HAL_I2C_IsDeviceReady(i2c, BMP180_I2C_ADDRESS, 10, 100)) {
	...
	HAL_I2C_Mem_Read(bmp_hi2c1, BMP180_I2C_ADDRESS, BMP180_REGISTER_EEPROM, 1, data, 22, 100);
...
TM_BMP180_Result_t TM_BMP180_StartTemperature(TM_BMP180_t* BMP180_Data) {
	uint8_t buffer[] = {BMP180_COMMAND_TEMPERATURE};
	/* Check for library initialization */
	if (!lib_initialized) {
		return TM_BMP180_Result_LibraryNotInitialized;
	}
	/* Send to device */
	HAL_I2C_Mem_Write(bmp_hi2c1, BMP180_I2C_ADDRESS, BMP180_REGISTER_CONTROL, 1, buffer, 1, 100);
	/* Set minimum delay */
...
TM_BMP180_Result_t TM_BMP180_ReadTemperature(TM_BMP180_t* BMP180_Data) {
	...
	HAL_I2C_Mem_Read(bmp_hi2c1, BMP180_I2C_ADDRESS, BMP180_REGISTER_RESULT, 1, data, 2, 100);
...
TM_BMP180_Result_t TM_BMP180_StartPressure(TM_BMP180_t* BMP180_Data, TM_BMP180_Oversampling_t Oversampling) {
	...
	/* Send to device */
	HAL_I2C_Mem_Write(bmp_hi2c1, BMP180_I2C_ADDRESS, BMP180_REGISTER_CONTROL, 1, command, 1, 100);
...
TM_BMP180_Result_t TM_BMP180_ReadPressure(TM_BMP180_t* BMP180_Data) {
	...
	/* Read multi bytes from I2C */
	HAL_I2C_Mem_Read(bmp_hi2c1, BMP180_I2C_ADDRESS, BMP180_REGISTER_RESULT, 1, data, 3, 100);
